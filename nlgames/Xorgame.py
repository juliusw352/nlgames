...
def cval2(self, reps: int) -> float:
		...
		for a_ans in range(2**q_0):
			for b_ans in range(2**q_1):
				...
				a_compact = np.reshape(a_strategy,(q_0_original, reps))
				b_compact = np.reshape(b_strategy,(q_1_original, reps))
				...
				# XOR games don't really need both strategies separately, but only the XOR of both,
				# so we can create a matrix that represents all the info we need.
				a_matrix = np.ndarray((reps, q_0_original, q_0_original))
				b_matrix = np.ndarray((reps, q_1_original, q_1_original))
				combined_matrix = np.ndarray((reps, q_0_original, q_1_original))
				for i in range(reps):
					
					a_matrix[i] = np.multiply(a_compact[i].T.reshape(-1,1), np.ones((1,q_0)))
					b_matrix[i] = np.multiply(b_compact[i].T.reshape(-1,1), np.ones((1, q_1))).T
					
					combined_matrix[i] = np.mod(a_matrix[i] + b_matrix[i], 2)
				success_matrix = combined_matrix == augmented_pred
				# reduced_matrix = np.multiply.reduce(success_matrix, 0)
				augmented_success = np.ndarray((q_0, q_1))
				for i in range(augmented_success.shape[0]):
					for j in range(augmented_success.shape[1]):
